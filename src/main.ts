...
const createIssueTemplateMutation = gql`
    mutation CreateIssueTemplate($name: String!, $description: String!, $issueTypes: [IssueTypeInput!]!, $issueStates: [IssueStateInput!]!, $issuePriorities: [IssuePriorityInput!]!, $relationTypes: [IssueRelationTypeInput!]!, $assignmentTypes: [AssignmentTypeInput!]!) {
        createIssueTemplate(input: {
            name: $name,
            description: $description,
...
async function createIssueTemplate(client: GraphQLClient, variables: any) {
    try {
        const response: any = await client.request(createIssueTemplateMutation, variables);
        console.log("Created issue template with ID:", response.createIssueTemplate.issueTemplate.id);
        return response.createComponentTemplate.issueTemplate;
    } catch (error) {
